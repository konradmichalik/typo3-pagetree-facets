 ...
 * ExampleTabListener.
 *
 * Registers the example facet through the exact same public event a real
 * third-party extension would use - no private shortcut, matching how the
 * main extension's own built-in facets register themselves.
 *
 * This is the whole registration: one #[AsEventListener] whose identifier only
 * has to be unique, and one addFacet() call. There is nothing to add to
 ...
 * above 100 would push the facet in front of the built-ins - possible, and
 * occasionally right, but not something to demonstrate as the norm. See
 * RegisterFacetsEvent's docblock for the priority convention.
 *
...
final readonly class ExampleTabListener
{
    ...
    {
        $event->addFacet($this->exampleTab);
    }
}
