 ...
 * `new AjaxRequest(url).withQueryArguments(args).get()` resolving to an object
 * with `.resolve()`. Tests drive it through `respondWith`/`failWith` rather than
 * by intercepting fetch, so no network layer is involved at all.
 *
...
/** Every request made since the last reset: `{url, args}`, in order. */
